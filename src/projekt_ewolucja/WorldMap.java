...
public class WorldMap {
    ...
    public WorldMap(Parameters p) throws IllegalArgumentException {
        parameters = p;
        if (p.worldHeight < p.jungleHeight) throw new IllegalArgumentException("Wysokosc dzungli musi byc mniejszy niz mapy");
        if (p.worldWidth < p.jungleWidth) throw new IllegalArgumentException("Rozmiar dzungli musi byc mniejszy niz mapy");
            ...
            if (atmpList != null) {
                if (atmpList.size() >= 2) {
                    ...
                    if (dog.canCopulate(cat)) {
                        ...
                        for(Vector2d posTmp : randPositions) if (this.animalMap.get(posTmp) != null && this.animalMap.get(posTmp).isEmpty()){
